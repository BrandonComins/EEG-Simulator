...
EEG::EEG(int id)
    ...
    , m_last_noise(0.0)
    , m_settings(std::make_unique<QSettings>(SettingsDefs::get_channel_settings_path(id),
                                             QSettings::IniFormat)) {
...
void EEG::initialize_from_settings() {
    ...
    if (!m_settings->contains(SettingsDefs::ChannelKeys::alpha_freq)) {
        ...
        m_settings->setValue(SettingsDefs::ChannelKeys::noise_scale,
                             SettingsDefs::ChannelDefaults::noise_scale);
        m_settings->setValue(SettingsDefs::ChannelKeys::persistence,
                             SettingsDefs::ChannelDefaults::persistence);
